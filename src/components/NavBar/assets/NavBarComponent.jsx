...
const NavBar = ({ Tabs, gap = '20px', display = 'flex', flexDirection = 'row', listStyleType = 'none', textDecoration = 'none', color = 'black', padding = '0px', hover = 'default', children }) => {
  ...
  const [tab, setTab] = useState(0); // Initialize counter state
  ...
  useEffect(() => {
    ...
    const extension = currentUrl.substring(currentUrl.lastIndexOf('/') + 1);
...
<Link
  ...
  style={{
    ...
    color: urlExtension === tab ? 'orange' : color // Color value should be a string
  }}
  className={hover}
  to={tab}
>
